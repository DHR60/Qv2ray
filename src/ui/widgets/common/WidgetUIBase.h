...
public:
    explicit QvStateObject(const QString &name)
        : windowName(name) {};
...
public:
    explicit QvDialog(const QString &name, QWidget *parent)
        : QDialog(parent), QvStateObject(name)
    {
#if QV2RAY_FEATURE(ui_has_store_state)
        connect(this, &QvDialog::finished, [this]
                {
                    SaveState();
                });
#endif
