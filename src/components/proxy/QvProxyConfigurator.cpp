...
void SetSystemProxy(const QString &address, int httpPort, int socksPort)
{
    ...
    if (hasHTTP)
    {
        ...
        for (const auto &protocol : QStringList{ "http", "ftp", "https" })
        {
            ...
            {
                actions << ProcessArgument{
                    "gsettings", { "set", "org.gnome.system.proxy." + protocol, "host", address }
                };
                actions << ProcessArgument{
                    "gsettings", { "set", "org.gnome.system.proxy." + protocol, "port", QSTRN(httpPort) }
                };
            }
    ...
    if (hasSOCKS)
    {
        // for GNOME:
        {
            ...
            actions << ProcessArgument{
                "gsettings", { "set", "org.gnome.system.proxy.socks", "port", QSTRN(socksPort) }
            };
