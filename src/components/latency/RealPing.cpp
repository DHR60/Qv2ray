...
struct RealPingContext
{
    ...
    RealPingContext(curl_socket_t sockfd, uvw::loop &loop)
        : handle{ loop.resource<uvw::poll_handle>(uvw::os_socket_handle{ sockfd }) }, sockfd{ sockfd }
    {
    }
};
...
static int handle_socket(CURL *easy, curl_socket_t s, int action, void *userp, void *socketp)
{
    ...
    case CURL_POLL_IN:
    case CURL_POLL_OUT:
        curl_context = socketp ? static_cast<RealPingContext *>(socketp)
                               : new RealPingContext{ s, pRealPingGlobalInfo->timer->parent() };

        if (!socketp)
            curl_multi_assign(pRealPingGlobalInfo->multiHandle, s, static_cast<void *>(curl_context));

        if (action == CURL_POLL_IN)
            events = uvw::poll_handle::poll_event_flags::READABLE;
        if (action == CURL_POLL_OUT)
            events = uvw::poll_handle::poll_event_flags::WRITABLE;

        curl_context->handle->on<uvw::error_event>(
            [sockfd = curl_context->sockfd, pRealPingGlobalInfo](uvw::error_event &, uvw::poll_handle &)
            {
        ...
        curl_context->handle->on<uvw::poll_event>(
            [sockfd = curl_context->sockfd, pRealPingGlobalInfo](uvw::poll_event &e, uvw::poll_handle &)
            {
                pRealPingGlobalInfo->timer->stop();
                int running_handles;
                int flags = 0;

                if (static_cast<bool>(e.flags & uvw::poll_handle::poll_event_flags::READABLE))
                    flags |= CURL_CSELECT_IN;
                if (static_cast<bool>(e.flags & uvw::poll_handle::poll_event_flags::WRITABLE))
