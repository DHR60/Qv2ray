...
void DokodemoDoorInboundEditor::SetContent(const QJsonObject &content)
{
    PLUGIN_EDITOR_LOADING_SCOPE({
        ...
        // Dokodemo-Door
        dokoFollowRedirectCB->setChecked(content.value(QStringLiteral("followRedirect")).toBool(false));
        dokoIPAddrTxt->setText(content.value(QStringLiteral("address")).toString(QString()));
        dokoPortSB->setValue(content.value(QStringLiteral("port")).toInt(0));
        dokoTimeoutSB->setValue(content.value(QStringLiteral("timeout")).toInt(0));
        const QString network = content.value(QStringLiteral("network")).toString();
        dokoTCPCB->setChecked(network.contains(QStringLiteral("tcp")));
        dokoUDPCB->setChecked(network.contains(QStringLiteral("udp")));
    })
}
...
void DokodemoDoorInboundEditor::on_dokoIPAddrTxt_textEdited(const QString &arg1)
{
    ...
    content.insert(QStringLiteral("address"), arg1);
}
...
#define SET_NETWORK                                       \
    ...
    if (hasUDP)                                           \
        list << "udp";                                    \
    content.insert(QStringLiteral("network"), list.join(","))
...
{
    PLUGIN_EDITOR_LOADING_GUARD
    content.insert(QStringLiteral("timeout"), arg1);
}

void DokodemoDoorInboundEditor::on_dokoFollowRedirectCB_stateChanged(int arg1)
{
    PLUGIN_EDITOR_LOADING_GUARD
    content.insert(QStringLiteral("followRedirect"), arg1 == Qt::Checked);
}
